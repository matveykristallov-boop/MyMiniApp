...
async def admin_give_stars(admin_id: str, target_user_id: str, amount: int):
    if admin_id not in ADMIN_IDS:
        raise HTTPException(403, "Not admin")
    db = await get_db()
    await db.execute("UPDATE users SET stars = stars + ? WHERE user_id = ?", (amount, target_user_id))
    await db.commit()
    await db.close()
    return {"status": "ok"}

@app.post("/api/admin/toggle_block")
async def admin_toggle_block(admin_id: str, target_user_id: str):
    if admin_id not in ADMIN_IDS:
        raise HTTPException(403, "Not admin")
    db = await get_db()
    async with db.execute("SELECT blocked FROM users WHERE user_id = ?", (target_user_id,)) as cursor:
        user = await cursor.fetchone()
    if not user:
        await db.close()
        raise HTTPException(404, "User not found")
    new_blocked = 0 if user['blocked'] else 1
    await db.execute("UPDATE users SET blocked = ? WHERE user_id = ?", (new_blocked, target_user_id))
    await db.commit()
    await db.close()
    return {"status": "ok", "blocked": new_blocked}
